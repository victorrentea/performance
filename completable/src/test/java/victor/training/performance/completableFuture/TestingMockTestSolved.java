...
public class TestingMockTestSolved {
    ...
    void aNotFound() throws Exception {
        ...
        Mockito.when(dependencyMock.fetchX(ID)).thenReturn(failedFuture(notFoundException));
        Mockito.when(dependencyMock.fetchY(ID)).thenReturn(completedFuture(new Y("y")));
    ...
    void aTimeout() throws Exception {
        Mockito.when(dependencyMock.fetchX(ID)).thenReturn(supplyAsync(()->new X("never"), delayedExecutor(600, MILLISECONDS)));
        Mockito.when(dependencyMock.fetchY(ID)).thenReturn(completedFuture(new Y("y")));
    ...
    void aSolo() throws Exception {
        Mockito.when(dependencyMock.fetchX(ID)).thenReturn(completedFuture(new X("SOLO")));
    ...
    void aAndB() throws Exception {
        Mockito.when(dependencyMock.fetchX(ID)).thenReturn(completedFuture(new X("x")));
        Mockito.when(dependencyMock.fetchY(ID)).thenReturn(completedFuture(new Y("y")));
